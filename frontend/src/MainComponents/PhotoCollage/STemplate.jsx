...
const STemplate = ({ columns, rowsPerColumn }) => {
  ...
  const [imageScales, setImageScales] = useState(Array(totalCells).fill(1));
    ...
    for (let colIndex = 0; colIndex < columns; colIndex++) {
        ...
        for (let rowIndex = 0; rowIndex < rowsPerColumn[colIndex]; rowIndex++) {
            const cellHeight = rowIndex === 0 ? (maxRows * 150) / rowsPerColumn[colIndex] : 150;
            gridItems.push(
                <Row key={cellIndex} style={{ height: cellHeight, width: '100%', overflowY: 'auto' }}>
                    <Col span={24} style={{ padding: 0, maxHeight: cellHeight, maxWidth: '100%',borderStyle: '1px groove black'  }}>
    ...
    for (let rowIndex = 0; rowIndex < rows; rowIndex++) {
        ...
        for (let colIndex = 0; colIndex < columnsPerRow[rowIndex]; colIndex++) {
            gridItems.push(
                <Col key={cellIndex} span={24 / columnsPerRow[rowIndex]} style={{ padding: 0, maxHeight: '150px', maxWidth: '100%' }}>
                    <div
                        style={{ overflow: 'auto' }}>
                        ...
                        {console.log(images[cellIndex], cellIndex, "images_cellindex")}
                        {images[cellIndex] && (
                            <div
                            className="image-container"
                            style={{
        ...
        gridRows.push(
            <Row key={rowIndex} style={{ height: '150px', width: '100%', overflowY: 'auto', }}>
                {gridItems}
            </Row>
        );
    }
