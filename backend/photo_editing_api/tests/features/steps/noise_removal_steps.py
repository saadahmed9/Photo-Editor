...
@given(u'Noise removal API is up and running')
def set_impl(context):
    context.url = 'http://127.0.0.1:8000/noise_removal/'
...
@when(u'User clicks on Noise removal operation')
def step_impl(context):
    context.res = requests.post(context.url, data=json.dumps(context.body), headers=context.headers)


@then(u'verify that Noise removal operation is working')
def step_impl(context):
