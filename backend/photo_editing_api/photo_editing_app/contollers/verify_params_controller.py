...
def verify_functionality_passed(request):
    ...
    functionality_list = ['passport_photo_size', 'background_change', 'photo_collage', 'noise_removal', 'format_change',
                          'resize', 'mosaic_maker','crop','pdf_maker','brightness_contrast', 'video_compression']
    if val not in functionality_list:
        raise SuspiciousOperation("Please select the function from the following list  ['passport_photo_size','background_change','photo_college','noise_removal','format_change','resize','mosaic_maker', 'video_compression']")
